...
import numpy as np
from g2p.g2p_transformer.modules import *
from g2p.g2p_transformer.hyperparams import Hyperparams as hp
    ...
    def forward(self, x, y):
        ...
        if self.hp.sinusoid:
            ...
            if self.hp.use_gpu:
                ...
                self.dec += self.dec_positional_encoding(
                    Variable(torch.unsqueeze(torch.arange(0, self.decoder_inputs.size()[1]), 0).repeat(
                        self.decoder_inputs.size(0), 1).long()))
